...
2. **领域模型**：快速回顾 `client`模块中的 `Message`、`MessageQueue`、`Topic`等核心类，建立模型概念。
...
1. 
   ...
   **事务消息**：在 `broker`模块中研究事务回查机制。

2. 

   **顺序消息**：理解在 `Broker`和 `Client`端如何保证顺序性。

3. 

   **新架构**：研究 `controller`和 `proxy`模块，理解其如何解耦原有架构，为云原生做准备。

### **第四阶段：实践与输出**

1. 

   **性能调优**：利用 `tools`模块的管理命令，结合源码知识，进行压测和调优。

2. 

   **问题定位**：模拟线上故障（如Broker宕机、网络分区），用源码知识分析现象并解决。

3. 

   **二次开发**：基于 `common`、`remoting`等模块，尝试定制一个简单的消息队列，加深理解。

4. 

   **参与社区**：关注 `github`目录下的工作流，尝试解决一个简单的Issue。
